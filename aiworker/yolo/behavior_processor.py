...
class AbnormalBehaviorProcessor:
    ...
    def __init__(self, camera_id: int, yolo_detector: YoloDetector, fps: int):
        ...
        self.warning_zones = {self.camera_id: zone_data.get('zones', [])}
        self.stay_frames_required = int(self.fps * zone_data.get('safe_time', DEFAULT_STAY_SECONDS))
        self.safe_distance = zone_data.get('safe_distance', DEFAULT_SAFE_DISTANCE)
    ...
    def process_frame(self, frame: np.ndarray) -> tuple[np.ndarray, dict]:
        ...
        if not kpts_list:
            ...
            return processed_frame, {}  # 没有检测到人，仅绘制区域后返回
