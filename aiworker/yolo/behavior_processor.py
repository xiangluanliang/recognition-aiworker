...
class AbnormalBehaviorProcessor:
    ...
    def process_frame(self, frame: np.ndarray) -> tuple[np.ndarray, dict]:
            ...
            if not kpts_list:
                if self.frame_idx % 100 == 0: self._cleanup_stale_ids(set())
            ...
            for pid in current_pids: self.person_last_seen[pid] = self.frame_idx
            for i, pid in enumerate(ids): self.center_histories[pid].append(centers[i])

            all_event_pids = set()

            # --- 打架检测 ---
            if 'fight_detection' in self.active_detectors:
                for i, pid in enumerate(ids): self.fight_kpts_history[pid].append(kpts_list[i].copy())
                conflict_pairs = detect_fight(
                    ids, centers, self.fight_kpts_history, self.center_histories,
                    dist_thresh=FIGHT_DISTANCE_THRESHOLD, motion_thresh=FIGHT_MOTION_THRESHOLD,
                    orient_thresh=FIGHT_ORIENTATION_SIMILARITY_THRESHOLD, speed_thresh=FIGHT_SPEED_THRESHOLD,
                    accel_thresh=FIGHT_ACCELERATION_THRESHOLD, kpts_change_thresh=FIGHT_KPTS_CHANGE_THRESHOLD
                )
                for pid1, pid2, fight_score in conflict_pairs:
                    all_event_pids.add(pid1)
                    all_event_pids.add(pid2)
            ...
            for i, (pid, kpts) in enumerate(zip(ids, kpts_list)):
                ...
                if 'fall_detection' in self.active_detectors:
                    ...
                    if is_new_fall:
                        # 无论有无音频，都先初始化 final_score 和 details
                        final_score = score
                ...
                if 'intrusion_detection' in self.active_detectors:
                    is_intruding, new_intrusion_zones_info = check_intrusion(
                        pid, bbox, centers[i], self.warning_zones,
                ...
                display_color = (0, 0, 255) if pid in all_event_pids else (0, 255, 0)
                # ▲▲▲ 修正结束 ▲▲▲

                # --- 可视化调试信息 (现在能正确显示入侵状态了) ---
                debug_texts = []
                if is_intruding:
                    # 查找此人对应的计时器信息
                ...
                # 在头顶绘制ID和调试文本
                label = f"ID:{pid}"
                cv2.putText(processed_frame, label, (bbox[0], bbox[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, display_color, 2)
